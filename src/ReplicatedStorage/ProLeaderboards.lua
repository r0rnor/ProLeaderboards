...
local function SetKeyInfo(self : Leaderboard, SetKey : string, NewValue : number, PreviousValue : number)
	local NewKeyInfo : KeyInfo = {
		Cell = self.LastCellIndex,
		Value = NewValue,
...
local function CalculateLeaderboardValue(self : Leaderboard, SetKey : string, NewValue : number)
	...
	if not KeyInfo then
		KeyInfo = SetKeyInfo(self, SetKey, NewValue, 0)
	end

	if KeyInfo.Cell < self.LastCellIndex then
		print("[😨] Updating KeyInfo OLD", KeyInfo)
		KeyInfo = SetKeyInfo(self, SetKey, NewValue, KeyInfo.Value)
